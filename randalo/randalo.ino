...
unsigned int chooseStart(){
  return map(mozziAnalogRead(A7),0,1023, 0,(unsigned int)ABOMB_NUM_CELLS);
}


unsigned int chooseEnd(unsigned int startpos){
  return map(mozziAnalogRead(A6),0,1023, 0,(unsigned int)ABOMB_NUM_CELLS);
}
...
void updateControl(){
  ...
  if(mozziAnalogRead(A0) > 512 && !gate){
    ...
    chooseSpeedMod();
    gate = true;
    aSample.start(startpos);
  }
  //playspeedmod += speedchange;
  aSample.setFreq((float) (mozziAnalogRead(A5)>>3)/127);
